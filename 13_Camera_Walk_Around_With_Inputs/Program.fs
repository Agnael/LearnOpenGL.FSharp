...
let main argv =
    ...
    let onMouseMove mouse (position: Vector2) = 
        ...
        let newCameraPitch = cameraPitch + (mouseYoffset * mouseSensitivity)
        cameraPitch <- 
            if newCameraPitch > 89.0f then 
                89.0f 
            else if newCameraPitch < -89.0f then 
                -89.0f 
            else 
                newCameraPitch
        ()
    ...
    let onUpdate dt =
        ...
        let newCamTargetX = MathF.Cos(toRadians cameraYaw) * MathF.Cos(toRadians cameraPitch)
        let newCamTargetY = MathF.Sin(toRadians cameraPitch)
        let newCamTargetZ = MathF.Sin(toRadians cameraYaw) * MathF.Cos(toRadians cameraPitch)
        cameraTarget <- 
